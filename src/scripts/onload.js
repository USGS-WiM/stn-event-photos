$( document ).ready(function() {
    ...
    // Get list of Sites
    var siteList = $.parseJSON($.ajax({
      url:  "https://stn.wim.usgs.gov/STNServices/Sites.json",
      dataType: "json",
      async: false
    }).responseText);
    ...
    var photoArray = [];
    var site_id = siteList[i].site_id;
    ...
    loadEventImages = function(event_id){
      ...
      var siteInfo = $.parseJSON($.ajax({
        url:  "https://stn.wim.usgs.gov/STNServices/Sites/"+site_id+"/Files.json",
        dataType: "json",
        async: false
      }).responseText);
      ...
      if(eventImages.length == 0){
        ...
        for ( var i = 0, l = eventImages.length; i < l; i++ ) {
          ...
          // HELP: breaks after it runs though a few objects. Need to account for site description to be undefined? 
          /* fileData.siteDescription = siteInfo[i].description; */
  
          fileData.siteCounty = siteList[i].county;
          fileData.siteState = siteList[i].state;
          /* fileData.agencyName = agencyList[i].agency_name; */
    ...
    showPhotoDetails = function(i){
      ...
      $("#imageDescription").text("Photo of " + photoArray[i].fileDescription + " at " + photoArray[i].siteDescription + " , " + photoArray[i].siteCounty + " , " + photoArray[i].siteState + " , " + photoArray[i].fileDate + ". "+ "Photographed by " + photoArray[i].fileDate + " , " + photoArray[i].agency_name);
